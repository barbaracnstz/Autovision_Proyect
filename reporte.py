...
# Cargar la imagen de fondo (opcional)
fondo_imagen = Image.open("Fondo.png")  # Cargar la imagen
fondo_imagen = fondo_imagen.resize((ventana.winfo_screenwidth(), ventana.winfo_screenheight()), Image.LANCZOS)  # Ajustar el tamaño a la pantalla
fondo = ImageTk.PhotoImage(fondo_imagen)  # Convertir a PhotoImage

# Crear un label para mostrar la imagen de fondo (opcional)
label_fondo = Label(ventana, image=fondo)
label_fondo.place(x=0, y=0, relwidth=1, relheight=1)  # Colocar el label ocupando toda la ventana

# Etiqueta de título centrada en la ventana
...
tipo_reporte = StringVar(value="multados")  # Asignar "multados" como valor por defecto
...
radio_multados.grid(row=1, column=1)

radio_residentes = Radiobutton(ventana, text="Reporte residentes", variable=tipo_reporte, value="residentes", bg='white')
radio_residentes.grid(row=1, column=2)

# Inicializar las fechas en los calendarios para mostrar todos los multados al abrir
hoy = datetime.now()
calendario_desde = DateEntry(ventana, width=12, background='darkblue', foreground='white', borderwidth=2, year=hoy.year, month=hoy.month, day=1)
calendario_desde.grid(row=2, column=2, padx=(0, 10))  # Añadir padding a la derecha

calendario_hasta = DateEntry(ventana, width=12, background='darkblue', foreground='white', borderwidth=2, year=hoy.year, month=hoy.month, day=hoy.day)
calendario_hasta.grid(row=2, column=4, padx=(0, 10))  # Añadir padding a la derecha

# Crear el marco para centrar el Treeview y su barra de desplazamiento 
frame = ttk.Frame(ventana)  
frame.grid(row=3,columnspan=5,padx=(200),pady=(100))  # Ajustar padding vertical y horizontal 

# Crear el Treeview para mostrar resultados 
tree = ttk.Treeview(frame , show="headings")
tree.pack(side="left")  # Colocar el Treeview a la izquierda

# Añadir barra de desplazamiento vertical
scroll = ttk.Scrollbar(frame, orient="vertical", command=tree.yview)
scroll.pack(side="right", fill="y")  # Colocar la barra de desplazamiento a la derecha
tree.configure(yscrollcommand=scroll.set)

# Botón para mostrar el reporte
boton_mostrar = Button(ventana, text="Mostrar reporte", command=lambda: mostrar_reporte_multados() if tipo_reporte.get() == "multados" else mostrar_reporte_residentes())
boton_mostrar.grid(row=4, column=0, padx=(0, 10), pady=(20))

# Botón para generar el archivo Excel
boton_excel = Button(ventana, text="Generar Excel", command=generar_excel)
boton_excel.grid(row=4, column=1, padx=(0, 10), pady=(20))

# Mostrar automáticamente los multados al iniciar la aplicación
